...
const Archeived_Posts = ({ posts, onUnarchive }) => {
  ...
  const handleUnarchivePost = (id) => {
    const postToUnarchive = posts.find(post => post.id === id);
  ...
  return (
    <div style={{
      ...
      {posts.length === 0 ? (
        <p style={{ color: '#4a2c2c', fontSize: '16px', textAlign: 'center' }}>No archived posts available.</p>
      ) : null}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#4a2c2c' }}>Archived Posts</h3>
              <button
                onClick={handlePopupToggle}
                style={{
      ...
      {selectedPost && <Pop_Up_PostDetails post={selectedPost} onClose={() => setSelectedPost(null)} onRestore={handleUnarchivePost} onDelete={() => {}} isDeleted={false} />}
    </div>
  );
};

export default Archeived_Posts;