...
import { AlertCircle, ArchiveRestore, Trash2, X, XCircle } from 'lucide-react';
...
export default function FlaggedPosts({ posts, onRestore, onError }) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {posts.length > 0 && (
          <span style={{ color: '#2C3E50', fontSize: '14px' }}>{posts.length} flagged {posts.length === 1 ? 'post' : 'posts'}</span>
        )}
      </div>


      {posts.length === 0 && (
        <p style={{ color: '#2C3E50', fontSize: '14px', textAlign: 'center', margin: "0px" }}>No flagged posts available.</p>
      )}
        ...
        <div style={MODAL_OVERLAY_STYLE} onClick={(e) => e.stopPropagation()}>
          <div style={{
            ...
            <div style={{ backgroundColor: '#EDEFF2', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 }}>
              <h3 style={{ margin: 0, fontSize: '16px', color: '#2C3E50' }}>Flagged Post Management</h3>
              <ButtonTrans
                child={<>
                  <X size={18} />
                </>}
