...
const savedController = async (req, res) => {
  ...
  try {
    if (!userId) {
      console.log('Missing userId in savedController');
    ...
    const result = await Blog
      .find({ savedBy: queryUserId, status: 'active' })
      .populate({
        path: 'user',
        select: 'user email',
    ...
    console.log(`Fetched ${sanitizedResult.length} saved blogs for userId: ${userId}`);
    res.status(200).json(sanitizedResult);
  } catch (err) {
    console.error('Saved blog error:', {
      message: err.message,
      stack: err.stack,
      query: req.query,
      timestamp: new Date().toISOString(),
    });
    res.status(500).json({ message: 'Error fetching saved blogs', error: err.message });
  }
};
